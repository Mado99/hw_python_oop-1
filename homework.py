class InfoMessage:
    ...
    def init(self, training_type: str, duration: float, distance: float, speed: float, calories: float):
...
class SportsWalking(Training):
    ...
    def init(self, action: int, duration: float, weight: float, height: float) -> None:
        super().init(action, duration, weight)
        self.height = height
...
class Swimming(Training):
    ...
    def init(self, action: int, duration: float, weight: float, length_pool: float, count_pool: int) -> None:
        super().init(action, duration, weight)
        self.length_pool = length_pool
        self.count_pool = count_pool
        self.LEN_STEP = 1.38  # Изменяем значение LEN_STEP для плавания
